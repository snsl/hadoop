...
class BlockReceiver implements java.io.Closeable, FSConstants {
  ...
  BlockReceiver(Block block, DataInputStream in, String inAddr,
    ...
    try{
      ...
      if (streams != null) {
        ...
        if(block.getBlockName().length() > 17)
        {
  ...
  private int receivePacket() throws IOException {
    ...
    if ((block.getBlockName().length() > 17 && len == 0) || (block.getBlockName().length() <= 17 && lastPacketInBlock)) {
      LOG.debug("Receiving empty packet for block " + block);
    } else {
      offsetInBlock += len;

      int checksumLen = (block.getBlockName().length() > 17 ? ((len + bytesPerChecksum - 1)/bytesPerChecksum)*checksumSize : checksumSize);

      if ( block.getBlockName().length() > 17 && (buf.remaining() != (checksumLen + len))) {
