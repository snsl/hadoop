...
public class TestDFSIO extends Configured implements Tool {
  ...
  private static final String BASE_FILE_NAME = "obj";
  ...
  public static class ReadMapper extends IOStatMapper<Long> {
    ...
    public Long doIO(Reporter reporter, 
      ...
      while ((output = combined_info_buffer.readLine()) != null)
      {
        ...
        token2 = st.nextToken();
        token3 = st.nextToken();
        if(token3.contains(name + "_") && hostname.contains(token2))
        {
      ...
      if (adjusted_path == null) {
        in = fs.open(new Path(DATA_DIR, name));
      } else {
      ...
      try {
        while (actualSize < totalSize) {
          ...
          if (adjusted_path == null) {
            ...
            curSize = adjustedIn.read(buffer, 0, bufferSize);
          }
          if (curSize < 0) break;
          actualSize += curSize;
