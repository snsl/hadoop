...
class FSDirectory implements FSConstants, Closeable {
   ...
   */
   private static HdfsFileStatus createFileStatus(byte[] path, INode node) {
    ...
    String size_info_path = home_path + "/global_file_SIZES";
    File global_size_info = new File (size_info_path);
    int fakesize = 0;
    String output = null, token1 = null, token2 = null;
    ...
    try {
      ...
      while ((output = size_file_buffer.readLine()) != null)
      {
        ...
        while (st.hasMoreTokens()) {
          token1 = st.nextToken();
          if (token1.equals(node.getLocalName()))
          {
            token2 = st.nextToken();
    ...
    return new HdfsFileStatus(
        node.isDirectory() ? 0 : ((token2 != null) ? fakesize :  ((INodeFile)node).computeContentSummary().getLength()), 
        node.isDirectory(), 
