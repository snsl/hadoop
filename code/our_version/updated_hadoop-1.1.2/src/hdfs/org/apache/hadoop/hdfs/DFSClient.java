...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  public class DFSInputStream extends FSInputStream {
    ...
    public synchronized void seek(long targetPos) throws IOException {
      ...
      int cut_from = src.lastIndexOf("/");
      int fakesize = 0;
      Path adjusted_path = null;
      ...
      while ((output = combined_info_buffer.readLine()) != null)
      {
        ...
        token2 = st.nextToken();
        token3 = st.nextToken();
        if(token3.contains(src.substring(cut_from + 1, src.length()) + "_"))
        {
          adjusted_path = new Path(token3);
          break;
        }
      }
        ...
        try {
          ...
          while ((output = size_file_buffer.readLine()) != null)
          {
            ...
            while (st.hasMoreTokens()) {
              token1 = st.nextToken();
              if (token1.equals(src.substring(cut_from + 1, src.length())))
              {
                token2 = st.nextToken();
                fakesize = Integer.parseInt(token2);
                break;
              }
            }
          }
      ...
      if (targetPos > (adjusted_path == null ? getFileLength() : fakesize)) {
