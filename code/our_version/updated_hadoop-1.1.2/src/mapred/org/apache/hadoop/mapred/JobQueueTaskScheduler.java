    ...
    for (int i=0; i < availableMapSlots; ++i) {
      synchronized (jobQueue) {
        for (JobInProgress job : jobQueue) {
          ...
          if (job.getJobConf().get("mapred.helper.path") != "" &&
            job.getJobConf().get("mapred.helper.path") != null) {
            String replica_locs = job.getJobConf().get("mapred.helper.path");
            String current_tt = taskTrackerStatus.getHost();
            StringTokenizer st = new StringTokenizer(replica_locs);
            String token = null;
            while (st.hasMoreTokens()) {
              token = st.nextToken();
              if (current_tt.contains(token)) {
                t =
                ...
                if (t != null) {
                  assignedTasks.add(t);
                  ++numLocalMaps;
                  if (job.desiredMaps() - numLocalMaps == 0) {
                    job.getJobConf().set("mapred.helper.path", "");
                  }
